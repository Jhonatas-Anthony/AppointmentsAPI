...
timeTableRouter.put('/cancel/:date/:hour', async (req, res) => {
  try {
    ...
    // Verificar se o agendamento existe e se pertence ao usuário logado

    if (a && regex === user) {
      const e = await TimeTable.updateOne({ time: req.params.hour, date: req.params.date }, { user: null });
      console.log(e)
      res.json(e)
    } else {
      throw Error("Operação inválida")
